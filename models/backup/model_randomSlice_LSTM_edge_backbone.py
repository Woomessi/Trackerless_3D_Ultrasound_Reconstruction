...
class Online_Finetuning_LSTM_Edge_Backbone(models.BaseModel):
    ...
    def custom_criterion(self, source, fake_gaps, fake_series, volume, bias):
        ...
        if weight_loss_consistency > 0:
            losses['loss_consistency'] = self._slice_consistency_loss(
                volume, bias_origin, H, W, H_c, W_c, crit_scale,
            )
    ...
    # ------------------------------------------------------------------
    # Arbitrary-plane sampler (used by Loss 5)
    # ------------------------------------------------------------------

    def _sample_arbitrary_pos(self, volume, bias_origin, H, W, device, dtype):
        """Sample a random probe-plane position that cuts through the volume.

        The center is placed uniformly within the inner 50% of each volume
        axis, and the orientation is drawn from an isotropic distribution.
        This guarantees the center is well inside the volume and that the
        plane intersects the volume over a large area regardless of pose.

        Args:
            volume       (D, H', W')  reconstructed 3-D volume (shape only used)
            bias_origin  (3,)         world-mm origin offset from reco()
            H, W         int          full-resolution image dimensions (pixels)
            device, dtype             target tensor device and dtype

        Returns:
            (arb_pos, ax_x, ax_y, ax_z)  if a qualifying plane is found:
                arb_pos  (1, 3, 3)  world-mm probe position [center, ll, lr]
                ax_x, ax_y, ax_z  (3,) orthonormal local frame unit vectors
            None  if no qualifying plane found within the search budget
        """
        vs        = volume.shape  # (D, H', W')
        half_w    = (W - 1) / 2.0 * self.scale_w
        half_h    = (H - 1) / 2.0 * self.scale_h
        threshold = float(getattr(self.cfg, 'intersect_threshold', 0.4))
        max_tries = int(getattr(self.cfg, 'arb_pos_max_tries', 30))

        b = bias_origin.detach().cpu().float()

        for _ in range(max_tries):
            # Center: uniformly from the inner 50% of each volume dimension
            cx = b[0].item() + vs[0] * (0.25 + 0.5 * torch.rand(1).item())
            cy = b[1].item() + vs[1] * (0.25 + 0.5 * torch.rand(1).item())
            cz = b[2].item() + vs[2] * (0.25 + 0.5 * torch.rand(1).item())
            center = torch.tensor([cx, cy, cz], device=device, dtype=dtype)

            # Random unit normal (plane normal = ax_z) via isotropic Gaussian
            raw  = torch.randn(3, device=device, dtype=dtype)
            ax_z = raw / raw.norm()

            # ax_x: horizontal axis orthogonal to ax_z
            ref  = torch.tensor([0.0, 0.0, 1.0], device=device, dtype=dtype)
            if ax_z[2].abs() > 0.9:
                ref = torch.tensor([1.0, 0.0, 0.0], device=device, dtype=dtype)
            ax_x = torch.linalg.cross(ref, ax_z)
            ax_x = ax_x / ax_x.norm()
            ax_y = torch.linalg.cross(ax_z, ax_x)
            ax_y = ax_y / ax_y.norm()

            # Probe position: [center, lower-left, lower-right]
            ll = center - ax_x * half_w - ax_y * half_h
            lr = center + ax_x * half_w - ax_y * half_h
            arb_pos = torch.stack([center, ll, lr]).unsqueeze(0)  # (1, 3, 3)

            frac = self._overlap_fraction(arb_pos, volume, bias_origin, H, W)
            if frac >= threshold:
                return arb_pos, ax_x, ax_y, ax_z
    ...
    def _slice_consistency_loss(
        self, volume, bias_origin, H, W, H_c, W_c, crit_scale,
    ):
        """Loss 5: spatial consistency via slightly rotated arbitrary-plane slices.

        Instead of anchoring to a ground-truth probe position, this method:
        1. Samples a random plane that cuts through the current partial 3-D
           volume with large overlap (arbitrary orientation; center placed in
           the inner 50 % of the volume bounding box).
        2. Extracts a volume slice at this arbitrary position → slice_orig.
        3. Applies a small fixed-magnitude tilt θ (random sign) around the
           plane's horizontal axis ax_x, producing a nearby rotated position.
        4. Extracts a volume slice at the rotated position → slice_rot.
        5. loss_consistency = L1(slice_orig, slice_rot)  or  (1 − SSIM).

        If the reconstructed volume is spatially coherent, neighbouring slice
        orientations yield similar appearances. Staircase artefacts caused by
        trajectory errors produce large differences.

        Gradient path
        ~~~~~~~~~~~~~
        bias_origin → arb_pos_vol / arb_pos_rot_vol
                    → get_slice (F.grid_sample ∂grid)
                    → slice_orig / slice_rot → loss_consistency

        Config keys
        ~~~~~~~~~~~
        weight_loss_consistency  float  loss weight (set > 0 to enable)
        consistency_angle        float  tilt magnitude in radians (default 0.01)
        consistency_loss_fn      str    'l1' (default) or 'ssim'
        arb_pos_max_tries        int    sampling retries (default 30)
        ...
        Returns:
            scalar Tensor  weighted consistency loss (0 if no plane found)
        """
        weight = float(getattr(self.cfg, 'weight_loss_consistency', 0.0))
        dev, dt = volume.device, volume.dtype

        # ── Sample an arbitrary plane that cuts through the volume ────────
        result = self._sample_arbitrary_pos(volume, bias_origin, H, W, dev, dt)
        if result is None:
            return torch.tensor(0.0, device=dev, dtype=dt) * weight

        arb_pos, ax_x, ax_y, ax_z = result

        # ── Slice at the arbitrary base position ──────────────────────────
        arb_pos_vol = arb_pos - bias_origin.view(1, 1, 3)
        slice_orig = my_utils.get_slice(
            volume, arb_pos_vol, (H_c, W_c),
            scale_h=self.scale_h / crit_scale,
            scale_w=self.scale_w / crit_scale,
        ).squeeze(0)  # (1, 1, H_c, W_c)

        # ── Rotated slice: tilt ax_y toward ax_z by fixed angle ──────────
        # 固定角度幅值，随机正负方向，覆盖两侧扰动且每步 loss 量级一致。
        ...
        ll_rot     = center - ax_x * half_w - ax_y_rot * half_h
        lr_rot     = center + ax_x * half_w - ax_y_rot * half_h
        arb_pos_rot     = torch.stack([center, ll_rot, lr_rot]).unsqueeze(0)  # (1, 3, 3)

        arb_pos_rot_vol = arb_pos_rot - bias_origin.view(1, 1, 3)
        slice_rot = my_utils.get_slice(
            volume, arb_pos_rot_vol, (H_c, W_c),
            scale_h=self.scale_h / crit_scale,
            scale_w=self.scale_w / crit_scale,
        ).squeeze(0)  # (1, 1, H_c, W_c)
