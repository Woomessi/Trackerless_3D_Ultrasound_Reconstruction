...
def read_calib_matrices(filename_calib):
    ...
    return torch.tensor(tform_calib[0:4, :]), torch.tensor(tform_calib[4:8, :]), torch.tensor(
        tform_calib[4:8, :] @ tform_calib[0:4, :])
...
def plotting(gt, frame, axs, step, color, width=4, scatter=8, legend_size=50, legend=None):
    ...
    for i_frame in range(0, gt.shape[0], step):
        ...
        for i, ax in enumerate(axs):
            ax.plot_surface(coord[0], coord[1], coord[2], facecolors=plt.cm.gray(pix_intensities), shade=False,
