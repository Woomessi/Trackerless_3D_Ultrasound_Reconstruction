...
class TUS(datasets.BaseDataset):
    ...
    def load(self):
        ...
        # Collect all h5 files: root/<subject>/<scan>.h5 未处理数据泄漏，各.h5文件单独从00到14编号，而不是(0,0)到(4,2)
        h5_files = sorted(glob.glob(os.path.join(root, '*', '*.h5')))
        if not h5_files:
            raise FileNotFoundError(f'No .h5 files found under {root}')

        # Split file list into train / val / test by ratio
        n_total = len(h5_files)
        ratio = self.cfg.train_val_test_split
        n_train = max(1, int(n_total * ratio[0]))
        n_val   = max(1, int(n_total * ratio[1]))
        n_test  = n_total - n_train - n_val
        if n_test < 0:
            n_val += n_test  # give up the last val scan instead
            n_test = 0

        # 实际上需要根据subject划分，而不是根据scan划分
        split_files = [
            h5_files[:n_train],
            h5_files[n_train:n_train + n_val],
            h5_files[n_train + n_val:],
        ]
