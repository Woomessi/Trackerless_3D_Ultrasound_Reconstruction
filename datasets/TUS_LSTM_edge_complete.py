...
__all__ = ['TUS_LSTM_edge_complete']


class TUS_LSTM_edge_complete(datasets.BaseDataset):
    ...
    def load(self):
        ...
        for files in split_files:
            for h5_path in files:
                ...
                # store path + frame count; frames are loaded on demand in __getitem__
                source_list.append({'path': h5_path, 'n_frames': n_frames})
    ...
    def __getitem__(self, index):
        ...
        meta = self.data['source'][idx]
        h5_path, N = meta['path'], meta['n_frames']
        target_point = self.data['target_point'][idx]

        frame_rate = torch.randint(self.cfg.frame_rate[0], self.cfg.frame_rate[1] + 1, (1,))
        n0 = random.randint(0, N - 2)
        idx_frames = [n0, n0 + 1]

        with h5py.File(h5_path, 'r') as f:
            frames_np = f['frames'][idx_frames]   # (2, H, W) uint8
