...
T_combined_np = T_combined.numpy()  # (N, 4, 4) float32, for scan-direction blending
...
class ProbeEnv:
    ...
    def __init__(
        ...
        plotter=None,
        axis_len: float = 50.0,
        scan_dir_blend: float = 1,
        blend_sigma_s: float = 0.25,
    ):
        """
        scan_dir_blend : float in [0, 1]
            Blend weight at the contact-point trajectory (s=0.5).
            0 → scanning direction = surface outward normal (current default).
            1 → scanning direction = true v-axis from T_combined (into body)
                at the nearest contact-trajectory frame, at s=0.5.
            The weight falls off as a Gaussian in s away from 0.5, so at
            s ≠ 0.5 the effective weight is
              w(s) = scan_dir_blend * exp(-0.5*((s-0.5)/blend_sigma_s)^2).
        blend_sigma_s : float
            Gaussian half-width in s that controls how quickly the blend
            decays as the probe moves laterally away from the contact
            trajectory (default 0.25 ≈ one quarter of the s range).
        """
        ...
        self._alen  = axis_len
        self._actors: list = []
        self._scan_dir_blend = float(scan_dir_blend)
        self._blend_sigma_s  = float(blend_sigma_s)
        self._T_comb_np      = T_combined_np  # (N, 4, 4) pixel→world transforms
    ...
    def _frame(self):
        ...
        The surface normal sign is anchored unconditionally to the T_combined
        v-axis so it always points outward (away from the body) even near
        surface boundaries where the ∂S/∂s × ∂S/∂t cross-product can flip.

        When scan_dir_blend > 0 the corrected outward normal is additionally
        blended toward the T_combined outward direction at the nearest contact-
        trajectory frame, weighted by a Gaussian in s centred at s=0.5."""
        origin_np = self._surf.evaluate(self.s, self.t).astype(np.float32)
        normal_np = self._surf.normal(self.s, self.t).astype(np.float32)

        # ── Anchor normal sign using T_combined ground truth ──────────────────
        # T_combined v-axis (col 1) points INTO the body; normal_np must be
        # anti-parallel (outward).  Near boundaries the cross-product sign can
        # flip, so correct it unconditionally before any further use.
        n_frames  = self._T_comb_np.shape[0]
        frame_idx = int(round(float(self.t) * (n_frames - 1)))
        frame_idx = max(0, min(n_frames - 1, frame_idx))
        v_col = self._T_comb_np[frame_idx, :3, 1].astype(np.float32)
        v_col /= (np.linalg.norm(v_col) + 1e-12)
        if np.dot(normal_np, v_col) > 0:   # normal is pointing into body — flip
            normal_np = -normal_np

        # ── Blend toward T_combined outward direction near contact trajectory ──
        if self._scan_dir_blend > 0.0:
            prox    = float(np.exp(-0.5 * ((float(self.s) - 0.5) / self._blend_sigma_s) ** 2))
            blend_w = self._scan_dir_blend * prox
            # -v_col: T_combined outward direction (same sign as corrected normal_np)
            blended = (1.0 - blend_w) * normal_np + blend_w * (-v_col)
            nrm = np.linalg.norm(blended)
            if nrm > 1e-12:
                normal_np = (blended / nrm).astype(np.float32)

        # ── Build probe frame axes ─────────────────────────────────────────────
        # Re-project the s-tangent reference onto the corrected normal's tangent
        # plane to keep ax_x exactly perpendicular to normal_np.
        ref = _ax_x_ref(self.s, self.t, surf=self._surf)
        ref = ref - normal_np * float(np.dot(normal_np, ref))
        nrm = np.linalg.norm(ref)
        if nrm > 1e-12:
            ref = (ref / nrm).astype(np.float32)
        ax_x_np = _rotate_around(ref, normal_np, self.angle)
