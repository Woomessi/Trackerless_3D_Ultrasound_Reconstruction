...
class ProbeEnv:
    ...
    Probe pose is constrained to the curved contact surface (surf_mesh):
      ...
      y-axis  – outward direction = -T_combined[v-axis]  (away from body)
      x-axis  – T_combined u-axis projected onto the y-plane, rotated by angle
      z-axis  – elevational direction  (ax_z = normal × ax_x)

    Scanning direction (-y) and lateral reference (x) come entirely from
    T_combined at the nearest frame index (determined by t), so the probe
    orientation is always consistent with the ground-truth acquisition geometry.

    State  : (s, t, angle)
    Action : (dx, dz, dθ) – translate along x / z (mm), rotate around y (rad)
    ...
    def _frame(self):
        ...
        Orientation is taken entirely from the nearest T_combined frame
        (determined by t):
          normal_np = -v_col  (outward = opposite of into-body v-axis)
          ax_x_np   = u_col projected onto the normal's tangent plane, then
                      rotated by self.angle around normal_np
          ax_z_np   = normal_np × ax_x_np  (elevational)
        """
        ...
        t_frame = self._T_comb_np[frame_idx]  # (4, 4)

        # v-axis (col 1): into-body scanning direction → outward = negation
        v_col = t_frame[:3, 1].astype(np.float32)
        v_col /= (np.linalg.norm(v_col) + 1e-12)
        normal_np = -v_col

        # u-axis (col 0): image lateral direction, projected onto tangent plane
        u_col = t_frame[:3, 0].astype(np.float32)
        u_col /= (np.linalg.norm(u_col) + 1e-12)
        u_col = u_col - normal_np * float(np.dot(normal_np, u_col))
        nrm = np.linalg.norm(u_col)
        if nrm > 1e-12:
            u_col = (u_col / nrm).astype(np.float32)

        ax_x_np = _rotate_around(u_col, normal_np, self.angle)
