...
# ── Extract 2D slices from the volume using get_slice ─────────────────────────
# get_slice expects series in volume-space: series_vol = series - bias
# series is in world-mm; bias shifts the origin so that voxel (0,0,0) = world min_point
#
# Query: extract slices for every frame using its original series position.
# Use the downsampled frame shape to match the volume's voxel resolution (1 voxel ≈ 1 mm).

# get_slice expects volume on the same device as series; both are on CPU here.

# volume = F.interpolate(volume.unsqueeze(0).unsqueeze(0), scale_factor=(1, 1 / scale_h, 1 / scale_w)).squeeze(0).squeeze(0)

slices = get_slice(volume, series - bias.unsqueeze(0).unsqueeze(0), source_down.shape[-2:])  # (1, N, 1, H_down, W_down)

slices = slices.squeeze(0,2)

slices = F.interpolate(
    slices.unsqueeze(1), scale_factor=(1/scale_h, 1/scale_w), mode='bilinear', align_corners=False
).squeeze(1)  # (N, H*scale_h, W*scale_w)

# ── Visualise a few reconstructed slices alongside the original frames ─────────
indices = [1,100,200,example_scan.shape[0]-1]

n_show = len(indices)
fig, axes = plt.subplots(2, n_show, figsize=(3 * n_show, 6), squeeze=False)
fig.suptitle("Top: original frame   |   Bottom: slice extracted from 3-D volume")

for col, idx in enumerate(indices):
    orig = source[idx].numpy()
    axes[0, col].imshow(orig, cmap="gray", vmin=0, vmax=1)
    axes[0, col].set_title(f"frame {idx}")
    axes[0, col].axis("off")

    rec_slice = slices[idx].numpy()
    axes[1, col].imshow(rec_slice, cmap="gray", vmin=0, vmax=1)
    axes[1, col].set_title(f"slice {idx}")
    axes[1, col].axis("off")
