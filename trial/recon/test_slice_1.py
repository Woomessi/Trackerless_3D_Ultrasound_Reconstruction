...
H_down, W_down = source_down.shape[-2], source_down.shape[-1]

series_vol = series - bias  # (N, 3, 3) – volume-space

# get_slice expects volume on the same device as series; both are on CPU here.
# Output shape: (1, N, 1, H, W)
slices = get_slice(volume, series_vol, (H_down, W_down))  # (1, N, 1, H_down, W_down)
slices = slices[0, :, 0, :, :]  # (N, H_down, W_down)

# ── Visualise a few reconstructed slices alongside the original frames ─────────
indices = [1,100,200,300]
