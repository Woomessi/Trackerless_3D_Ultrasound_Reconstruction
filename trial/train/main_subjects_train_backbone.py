...
class Main(object):
    ...
    def __init__(self):
        self.model_cfg = configs.BaseConfig('/home/wu/Documents/projects/cloned_repositories/RecON/res/models/online_bk.json')
    ...
    def my_split(self):
        ...
        self.dataset_all = Dataset(
            data_path='/home/wu/Documents/projects/cloned_repositories/RecON/data/frames_transfs',
            num_samples=2,
            sample_range=2
        )

        ## split the entair dataset into Train, Val, and Test
        self.dset_folds = self.dataset_all.partition_by_ratio(
            ratios=[1] * 5,
            randomise=True,
        )

        # save the indices of the splited train,val and test dataset, for reproducibility
        for (idx, ds) in enumerate(self.dset_folds):
            ds.write_json(os.path.join('/home/wu/Documents/projects/cloned_repositories/RecON/save', "fold_{:02d}.json".format(idx)))

        # construct the train, val and test dataset
        self.dset_train = self.dset_folds[0] + self.dset_folds[1] + self.dset_folds[2]
        self.dset_val = self.dset_folds[3]
        self.dset_test = self.dset_folds[4]
    ...
    def my_test(self, epoch, data_loader=None, log_text=None):
        utils.common.set_seed(int(time.time()) + epoch)
        torch.cuda.empty_cache()
        predict, count = {}, 0
        data_loader = data_loader or self.test_loader
        ...
        with torch.no_grad():
            ...
            epoch_info = {'epoch': epoch, 'batch_per_epoch': batch_per_epoch, 'count_data': count_data, 'log_text': log_text}
            for batch_idx, (sample_dict, index) in enumerate(data_loader):
            # for batch_idx, (frames, tforms, _, _) in enumerate(data_loader):
                _count = len(list(sample_dict.values())[0])
                epoch_info['batch_idx'] = batch_idx
                epoch_info['index'] = index
                epoch_info['batch_count'] = _count
    ...
    def val_test(self, epoch):
        self.my_test(epoch, data_loader=self.val_loader, log_text='Val')
        self.my_test(epoch, data_loader=self.test_loader, log_text='Test')
...
def run():
    ...
    # main.split()
    main.my_split()
    # for step, (frames, tforms, _, _) in enumerate(main.train_loader):
    #     print(step)
