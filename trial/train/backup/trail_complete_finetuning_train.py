...
class Main(object):
    ...
    def __init__(self):
        ...
        self.dataset_cfg = datasets.functional.common.more(
            configs.BaseConfig(
                '/home/wu/Documents/projects/cloned_repositories/RecON/res/datasets/TUS_complete.json'
            )
        )
    ...
    def split(self):
        ...
        self.run_cfg.test_batch_size = getattr(
            self.run_cfg, 'test_batch_size', self.run_cfg.batch_size
        )
    ...
    def train(self, epoch):
        ...
        count_data = len(self.train_loader.dataset)
        log_step = 1
        epoch_info = {
            'epoch': epoch,
    ...
    def test(self, epoch, data_loader=None, log_text=None):
        ...
        with torch.no_grad():
            ...
            log_step = (
                max(int(np.power(10, np.floor(np.log10(batch_per_epoch / 10)))), 1)
                if batch_per_epoch > 0 else 1
            )
            epoch_info = {
                'epoch': epoch,
                'batch_per_epoch': batch_per_epoch,
            ...
            for batch_idx, (sample_dict, index) in enumerate(data_loader):
                ...
                if batch_idx % log_step == 0:
                    self.logger.info(
                        '{} Epoch: {} [{}/{} ({:.0f}%)]'.format(
                            log_text, epoch, count, count_data,
                            100. * count / count_data
                        )
                    )
    ...
    def plot_loss_curve(self):
        ...
        fig.suptitle('Fine-tuning Loss Curves', fontsize=14)
        plt.tight_layout()
