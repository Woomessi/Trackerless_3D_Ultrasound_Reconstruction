...
# ── Functions ────────────────────────────────────────────────────────────────────
def _build_series(tforms_np, H_img, W_img):
    ...
    _, tform_calib_R_T, tform_calib = read_calib_matrices(CALIB_PATH)
...
tform_calib_scale, tform_calib_R_T, tform_calib = read_calib_matrices(CALIB_PATH)

# ── Plot 3D US Rectangle ──────────────────────────────────────────────────────────────────
...
# plot_scan(labels_four.numpy(),example_scan,os.getcwd()+'/'+"img"+'/'+"example",step=50,color = 'g',width = 4, scatter = 8, legend_size=50,legend = 'GT')

# ── Forward Inference ──────────────────────────────────────────────────────────────────
N, H_img, W_img = example_scan.shape[0], example_scan.shape[1], example_scan.shape[2]

# source_list, target_point_list, target_dof_list = [], [], []

source = torch.from_numpy(example_scan.astype(np.float32) / 255.0)
...
series = _build_series(example_transformation, H_out, W_out)

tp, td = preprocessing(series.reshape(-1, 9))
...
# real_source = source.to(device)
# real_source = real_source.unsqueeze(1)
# real_source = real_source.unsqueeze(0)
#
# real_target = td.to(device).squeeze(0)
#
# input = torch.cat([real_source[:, :-1, ...], real_source[:, 1:, ...]], dim=2)
#
# backbone = Backbone(2, 15 - 9).to(device)
# state = torch.load(CKPT_PATH, map_location=device)
# backbone.load_state_dict(state)
# backbone.eval()
#
# # Should I infer in mini-batches ?
# fake_gaps, _ = backbone(input)
#
# start_frame = tp[0:1]                                     # (1, 3, 3)
...
start_frame = tp[0:1].view(1, 3, 3)                 # (1, 3, 3)
pred_series = dof_to_series(
    start_frame,                                          # (1, 3, 3)
    fake_gaps.unsqueeze(0),                               # (1, N-1, 6)
)                                                          # (N, 3, 3)
