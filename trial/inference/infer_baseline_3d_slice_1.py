...
# ── Paths ─────────────────────────────────────────────────────────────────────
# H5_PATH    = "/media/wu/Extreme SSD/datasets/11178509/train_part1/024/LH_Par_L_PtD.h5"
...
def main():
    ...
    # Rotate around the width axis e_x: e_y tilts toward e_z (0° → 90°)
    n_steps = 10
    angles  = torch.linspace(0, torch.pi / 2, n_steps)
    ...
    for theta in angles:
        ...
        rotated_frames.append(torch.stack([C, LL_new, LR_new]))  # (3, 3)

    rotated_series = torch.stack(rotated_frames)           # (n_steps, 3, 3)
    ...
    ### Visualise slices ###
    ########################
    deg_labels = [f"{int(round(a.item() * 180 / torch.pi))}°" for a in angles]

    fig, axes = plt.subplots(2, n_steps, figsize=(n_steps * 2.5, 5), squeeze=False)
    fig.suptitle("Out-of-plane rotation: transverse → longitudinal (0° → 90°)")

    # Row 0: original frame at idx_start_point for reference (repeated)
    ...
    # Row 1: slices extracted from the 3D volume at each rotation angle
    for i in range(n_steps):
        axes[1, i].imshow(rot_slices[i], cmap="gray", vmin=0, vmax=1)
        axes[1, i].set_title(deg_labels[i], fontsize=8)
        axes[1, i].axis("off")
