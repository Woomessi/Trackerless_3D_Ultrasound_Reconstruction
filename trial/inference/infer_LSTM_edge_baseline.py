...
# CKPT_EPOCH = 36  # set to None to use the latest checkpoint
CKPT_EPOCH = 21  # set to None to use the latest checkpoint

OUT_DIR    = "../../infer_LSTM_edge_baseline_output"

# ── Model config (must match training) ────────────────────────────────────────
IN_PLANES   = 4   # [frame_i, frame_{i+1}, edge_i, edge_{i+1}]
...
def main():
    ...
    # 3b. Compute edge maps ────────────────────────────────────────────────────
    print("Computing edge maps ...")
    edge = utils.image.get_edge(source.squeeze(1).to(device), device=device)  # (N, 1, H, W)
    print(f"  edge shape: {edge.shape}")

    # 4. Build pair inputs: (N-1, 4, H, W) ────────────────────────────────────
    # Model expects (B, T, C, H, W) where C=4 = [frame_i, frame_{i+1}, edge_i, edge_{i+1}]
    pairs = torch.cat([source[:-1], source[1:], edge[:-1].cpu(), edge[1:].cpu()], dim=1)  # (N-1, 4, H, W)
    print(f"  pair input shape: {pairs.shape}")

    # 5. Load model ────────────────────────────────────────────────────────────
    if CKPT_EPOCH is not None:
        ckpt_path = os.path.join(CKPT_DIR, f'online_LSTM_edge_bk_backbone_{CKPT_EPOCH}.pth')
        if not os.path.exists(ckpt_path):
            raise FileNotFoundError(f"Checkpoint not found: {ckpt_path}")
    else:
        ckpt_files = sorted(
            glob.glob(os.path.join(CKPT_DIR, 'online_LSTM_edge_bk_backbone_*.pth')),
    ...
    with torch.no_grad():
        for start in range(0, N - 1, BATCH_PAIRS):
            ...
            batch = pairs[start:end].unsqueeze(0).to(device)  # (1, end-start, 4, H, W)
...
def visualize(gt, pred, out_dir):
    ...
    # Unified axis range for XY / XZ planes
    ...
    # Centered equal-aspect limits for YZ plane
    yz_pts  = np.vstack([gt_center[:, 1:3], pred_center[:, 1:3]])
    y_c     = (yz_pts[:, 0].min() + yz_pts[:, 0].max()) / 2
    z_c     = (yz_pts[:, 1].min() + yz_pts[:, 1].max()) / 2
    half_yz = max(yz_pts[:, 0].max() - yz_pts[:, 0].min(),
                  yz_pts[:, 1].max() - yz_pts[:, 1].min()) / 2 * 1.1
    yz_xlim = (y_c - half_yz, y_c + half_yz)
    yz_ylim = (z_c - half_yz, z_c + half_yz)
